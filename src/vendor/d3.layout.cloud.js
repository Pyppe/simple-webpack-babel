...
module.exports = function() {
    ...
    function step() {
      ...
      while (Date.now() - start < timeInterval && ++i < n && timer) {
        ...
        var isPlaced = (function() {
          var iterations = 5;
          return _.isNumber(
            _.chain(_.range(iterations)).
              find(function(i) {
                d.x = (size[0] * (random() + .5)) >> 1;
                d.y = (size[1] * (random() + .5)) >> 1;
                cloudSprite(contextAndRatio, d, data, i);
                var res = place(board, d, bounds, i === iterations-1);
                //console.log('iteration %s for %s yielded %s', i, d.text, res);
                return res;
              }).
              value()
          );
        ...
        if (!isPlaced) {
          //console.error('SKIP: ' + d.text);
        }
        if (d.hasText && isPlaced) {
  ...
  function place(board, tag, bounds, forceOverflow) {
    ...
    while (dxdy = s(t += dt)) {
      ...
      if (tag.x + tag.x0 < 0 || tag.y + tag.y0 < 0 ||
        ...
        if (forceOverflow !== true) continue;
      }
